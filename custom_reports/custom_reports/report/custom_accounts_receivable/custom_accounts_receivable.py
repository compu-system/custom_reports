...
def execute(filters=None):
	...
	return CustomReceivablePayableReport(filters).run(args)

class CustomReceivablePayableReport(object):
	...
	def get_currency_fields(self):
		return ['invoiced', 'paid', 'credit_note', 'outstanding','notdue', 'range1',
	...
	def get_ageing_data(self, entry_date, row):
		...
		if row.age < 0 :
				row.notdue = row.outstanding

		index = None

		if not (self.filters.range1 and self.filters.range2 and self.filters.range3 and self.filters.range4 and self.filters.range5 and self.filters.range6 and self.filters.range7 ):
			self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4 , self.filters.range5, self.filters.range6, self.filters.range7 = 30, 60, 90, 120, 180 , 365, 730

		for i, days in enumerate([self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4 , self.filters.range5, self.filters.range6, self.filters.range7]):
			if row.age <= days:
				index = i
				break

		if index is None: index = 8
	...
	def setup_ageing_columns(self):
		...
		self.add_column(_('NotDue'), fieldname='notdue')

		for i, label in enumerate(["0-{range1}".format(range1=self.filters["range1"]),
			"{range1}-{range2}".format(range1=cint(self.filters["range1"])+ 1, range2=self.filters["range2"]),
			"{range2}-{range3}".format(range2=cint(self.filters["range2"])+ 1, range3=self.filters["range3"]),
			"{range3}-{range4}".format(range3=cint(self.filters["range3"])+ 1, range4=self.filters["range4"]),
			"{range4}-{range5}".format(range4=cint(self.filters["range4"])+ 1, range5=self.filters["range5"]),
			"{range5}-{range6}".format(range5=cint(self.filters["range5"])+ 1, range6=self.filters["range6"]),
			"{range6}-{range7}".format(range6=cint(self.filters["range6"])+ 1, range7=self.filters["range7"]),
			"{range7}-{above}".format(range7=cint(self.filters["range7"])+ 1, above=_("Above"))]):
