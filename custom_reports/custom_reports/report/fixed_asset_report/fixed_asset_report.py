...
def get_data(filters):
	...
	for asset in assets_record:
		...
		accumulated_depreciation =  depreciation_amount_map.get(asset.asset_id) or 0.0
		row = {
			...
			"purcahse_invoice": asset.purchase_invoice,
			"vendor_name": pr_supplier_map.get(asset.purchase_receipt) or pi_supplier_map.get(asset.purchase_invoice),
			"gross_purchase_amount": asset.gross_purchase_amount,
			"opening_accumulated_depreciation": asset.opening_accumulated_depreciation,
			"depreciated_amount": accumulated_depreciation,
			"available_for_use_date": asset.available_for_use_date,
			"location": asset.location,
			"asset_category": asset.asset_category,
			"purchase_date": asset.purchase_date,
			"asset_value": asset_value,
			"net":  asset.gross_purchase_amount - accumulated_depreciation
		}
		data.append(row)
